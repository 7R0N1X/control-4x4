...
export const createUserWithEmailAndPasswordThunk = ( userName: string, userEmail: string, userPassword: string ) => {
  return async (dispatch: AppDispatch) => {
    try {
      ...
      error instanceof Error && dispatch(setLogIn({ errorMessage: error.message }));
    }
  };
};
...
export const logOut = () => {
  return async (dispatch: AppDispatch) => {
    try {
      ...
      error instanceof Error && dispatch(setLogOut({ uid: null, displayName: null, email: null, photoURL: null, errorMessage: error.message }));
    }
  };
};
